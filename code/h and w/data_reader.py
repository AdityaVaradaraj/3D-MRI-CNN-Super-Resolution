...
class DataReader:
    ...
    def __init__(self, train_path, eval_path, test_path, is_training=True, SHOW_IMAGES=False): 
        ...
        if is_training:
            self.train_images_in = utils.read_all_patches_from_directory(train_path, 'input_train_%d_%d' % (params.dim_patch, params.scale)) 
            self.train_images_gt = utils.read_all_patches_from_directory(train_path, 'gt_train_%d_%d' % (params.dim_patch, params.scale )) 
